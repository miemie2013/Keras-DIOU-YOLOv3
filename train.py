...
def loss_layer(conv, pred, label, bboxes, stride, num_class, iou_loss_thresh, alpha=0.5, gamma=2):
    ...
    # pos_loss = respond_bbox * (0 - K.log(pred_conf)) * K.pow(1 - pred_conf, gamma)
    # neg_loss = respond_bgd  * (0 - K.log(1 - pred_conf)) * K.pow(pred_conf, gamma)

    # RetinaNet的focal_loss，多带上alpha解决不平衡问题。
    # 经过试验发现alpha取>0.5的值时mAP会提高，但误判（False Predictions）会增加；alpha取<0.5的值时mAP会降低，误判会降低。
    # pos_loss = respond_bbox * (0 - K.log(pred_conf)) * K.pow(1 - pred_conf, gamma) * alpha
    # neg_loss = respond_bgd  * (0 - K.log(1 - pred_conf)) * K.pow(pred_conf, gamma) * (1 - alpha)
